...
Choose a local or remote trainer host using the [hosting guide](hosting.md). A Kubernetes cluster is a lab target; it is not required to host the terminal application.
...
Expected results are `yes`, `no`, `no`. The runner's normal command interface further restricts even namespaced access to authored commands. The example runner pins kubectl to v1.31.14 and checked against the upstream SHA-256 during build. Change the Docker build argument deliberately for another cluster version; the provided image build currently targets Linux amd64.

Set `SRE_LAB_NODE` in the private `.env` to constrain Kubernetes fixtures to a tested worker. This applies required node affinity to both web and client while leaving the Pending scenario's separate bad selector observable and repairable. Leave it blank to let the scheduler choose eligible nodes. Verify creation, recovery, and cleanup on the selected worker before using it for practice.
